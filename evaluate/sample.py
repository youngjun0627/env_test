...
class File_Evaluator():
    def __init__(self, file_name, index_columns=[]):
        ...
        OUTPUT_DIR = os.path.join(NOTEBOOK_DIR, 'work')
    ...
    def validate_check_file_exists(self):
        if not os.path.exists(self.file_path):
            return False, "파일 {self.file_name}가 존재하지 않습니다."
        return True, None
    ...
    def validate_check_columns_count(self):
        ...
        MESSAGE_COLUM_DIFFERENT = """파일 {self.file_name}의 컬럼이 예상과 다릅니다.
...
제출한 파일의 컬럼: {', '.join(cols_got)[:64]}"""
    ...
    def validate_check_rows_count(self):
        ...
        MESSAGE_ROW_DIFFERENT = """파일 {self.file_name}의 row 수가 예상과 다릅니다.
...
제출한 파일의 row 수: {rows_count_got+1}"""
    ...
    def evaluate(self):
        ...
        return self.calc_roc_auc_score() # 사용할 metric으로 return
